...
void render()
{
	...
	for (int j = 0; j < Height; ++j)
	{
		for (int i = 0; i < Width; ++i)
		{
			...
			for (int k = 0; k < N; ++k) {
				Ray* ray = camera.getAntialiasingRay(i, j);
				vec3 color = scene.trace(ray, 0.0f, INT_MAX);

				resultColor += color;
				// Clean up the dynamically allocated ray
				delete ray;
			}
			// Apply box filter with amplitude 1/N
			resultColor /= N;

			// Apply gamma correction
			resultColor.r = pow(resultColor.r, invGamma);
			resultColor.g = pow(resultColor.g, invGamma);
			resultColor.b = pow(resultColor.b, invGamma);



			// set the color
			OutputImage.push_back(resultColor.x); // R
			OutputImage.push_back(resultColor.y); // G
			OutputImage.push_back(resultColor.z); // B
		}
	}
}
